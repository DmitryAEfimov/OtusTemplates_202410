...
public class SqrtTest {
    ...
    public void testSqrt_whenAIsNearZero_shouldThrowException(double coefficient) {
        var sqrt = new Sqrt();
        assertThrows("Коэффициент a не должен быть равен 0",
                     IllegalArgumentException.class,
                     () -> sqrt.solve(coefficient, 1, 1, EPSILON));
    }

    @Test(dataProvider = "nearZeroProvider")
    public void testSqrt_whenDiscriminantIsLessThenZero_shouldThrowException(double coefficient) {
        var sqrt = new Sqrt();
        assertThrows("Решение невозможно",
                     IllegalArgumentException.class,
                     () -> sqrt.solve(1. - coefficient, coefficient, 1. + coefficient, EPSILON));
    }
    ...
    public void testSqrt_whenNotAFinite_shouldThrowException(double a, double b, double c, double epsilon) {
        var sqrt = new Sqrt();
        assertThrows("Все коэффициенты должны быть исчисляемыми",
                     IllegalArgumentException.class,
                     () -> sqrt.solve(a, b, c, epsilon));
    }
